...
export function MagicCard({ children, className, gradientColor = '#6272f6', gradientOpacity = 0.1, ...props }: React.HTMLAttributes<HTMLDivElement> & { gradientColor?: string; gradientOpacity?: number }) {
  ...
  return (
    <div ref={ref}
      ...
      className={cn('relative overflow-hidden rounded-xl border border-border bg-card shadow-sm transition-shadow duration-200', hovered && 'shadow-md shadow-brand-500/10', className)} {...props}>
