 ...
 * We then refresh the JWT — the new claims land in the auth store, and every
 * permission-driven piece of UI (sidebar, guards, buttons) re-renders live.
...
async function loop() {
  ...
  for (;;) {
    ...
    try {
      ...
      await streamAuthEvents(handleEvent, abort.signal)
