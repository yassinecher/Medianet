...
import { ArrowLeft, CheckCircle2, Loader2, AlertTriangle, Send, Star, ClipboardCheck } from 'lucide-react'
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // My existing evaluation (prefill + read-only summary) + my assignment phase.
        const mine = (c.evaluations ?? []).find((e: any) => e.juryId === user?.id) ?? null
        const myAssign = (c.juryAssignments ?? []).find((a: any) => a.juryId === user?.id) ?? null
        ...
        if (c.programmeId) {
          ...
          const phaseId = myAssign?.phaseId ?? c.phaseId
          if (phaseId) {
            try {
              const phase = ((await programmesApi.phases(c.programmeId)).data ?? []).find((p: any) => p.id === phaseId)
              if (phase) {
