package com.medianet.programme.security;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            // Don't log GETs (too noisy on public endpoints); only writes.
            if (!"GET".equals(request.getMethod())) {
                log.debug("No Authorization header on {} {} — request will be anonymous",
        ...
        try {
            if (jwtService.isTokenValid(token)) {
                ...
                if (!"GET".equals(request.getMethod())) {
                    log.info("JWT OK on {} {} — user={} authorities={}",
                            request.getMethod(), request.getRequestURI(), email, authorities);
                }
            ...
            log.warn("JWT invalid on {} {} — {}", request.getMethod(), request.getRequestURI(), e.getMessage());
        }
